...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("footer-more-stats", {
		...
		handler: async (args, ctx) => {
			...
			requestFooterRender?.();
			ctx.ui.notify(`Footer stats ${showMoreStats ? "shown" : "hidden"}`, "info");
		},
	});
			...
			function render(width: number): string[] {
				...
				for (const entry of ctx.sessionManager.getEntries()) {
					...
					if (entry.type === "message" && (entry.message.role === "assistant" || entry.message.role === "toolResult")) {
						usage = (entry.message as AssistantMessage | ToolResultMessage).usage;
				...
				const percentLabel =
					contextUsage?.percent !== null ? `${percent.toFixed(1)}%/${formatTokens(contextWindow)}` : `?/${formatTokens(contextWindow)}`;
				let contextColored: string;
				if (percent >= 80) contextColored = theme.fg("error", percentLabel);
				else if (percent >= 60) contextColored = theme.fg("warning", percentLabel);
				...
				if (cost > 0) suffix += ` ${theme.fg("dim", `$${cost < 0.01 ? cost.toFixed(3) : cost.toFixed(2)} •`)}`;
				...
				let modelSide = theme.fg("muted", ctx.model?.name || ctx.model?.id || "no-model");
				if (ctx.model?.reasoning) {
					const level = pi.getThinkingLevel() || "off";
					const thinkingSuffix = level === "off" ? " • thinking off" : ` • ${level}`;
					modelSide += theme.fg(THINKING_COLORS[level] ?? "dim", thinkingSuffix);
				}
				const activeAgent = statuses.get("active-agent");
				const agentPrefix = activeAgent ? `${activeAgent}${theme.fg("dim", " • ")}` : "";
				...
				const fits = (l: string, r: string) => visibleWidth(l) + 2 + visibleWidth(r) <= contentWidth;
				...
				if (leftWidth + 2 + rightWidth <= contentWidth) {
					mainLine = usedLeft + theme.fg("dim", " ".repeat(contentWidth - leftWidth - rightWidth)) + usedRight;
				} else {
					const availableForLeft = contentWidth - rightWidth - 2;
					if (availableForLeft > 0) {
						const truncatedLeft = truncateToWidth(usedLeft, availableForLeft, theme.fg("dim", "..."));
				...
				if (showMoreStats) {
					...
					const statusBits = Array.from(statuses.entries())
						...
						.map(([key, text]) => {
							...
							const connected = plain.match(/(\d+)\s+servers?\s+(?:enabled\b[^(]*)?\((\d+)\s+connected\)/i);
							if (connected) return theme.fg("accent", `🔌 ${connected[2]}/${connected[1]}`);
							const count = plain.match(/(\d+)/);
							return theme.fg("accent", count ? `🔌 ${count[1]}` : plain.replace(/^MCP:?\s*/i, ""));
						})
						.filter((bit) => visibleWidth(bit) > 0);
					const secondBits: string[] = [];
					if (tokenBits.length > 0) secondBits.push(theme.fg("dim", tokenBits.join(" ")));
					if (statusBits.length > 0) secondBits.push(statusBits.join(" "));
					const secondLine = secondBits.join(theme.fg("dim", " • "));
					if (secondLine) {
						lines.push(padFooterLine(truncateToWidth(secondLine, contentWidth, theme.fg("dim", "...")), width));
					}
				}
				return lines;
			}
