...
export function normalizeNativeCompactedWindowForReplay(
  compactedWindow: readonly unknown[],
): unknown[] | undefined {
  return cloneOpaqueCompactedWindow(compactedWindow);
}
...
function isNativeCompactionDisplayCustomMessageEntry(
...
function createReplayMessageSet<TApi extends Api>(
  model: Model<TApi>,
  messages: AgentMessage[],
): ReplayMessageSet {
  return {
...
function createReplayVariants<TApi extends Api>(args: {
  ...
  return [
    createReplayMessageSet(args.model, collectReplayMessages(args.entries)),
  ];
}
...
function clonePayloadConversationInput(args: {
  ...
  const tailEndIndex =
    args.payloadInput.length - args.freshPreamble.trailingInput.length;
  ...
  return cloneResponsesInputSlice(
    args.payloadInput.slice(
      args.freshPreamble.leadingInput.length,
      tailEndIndex,
    ),
  );
}
...
function buildLenientNativeReplayPayload(args: {
  ...
  compactedWindow: readonly unknown[];
  compactionSummaryInput: readonly ResponsesInputItem[];
  preCompactionInput: readonly ResponsesInputItem[];
  ...
  let replayConversationInput = stripLeadingCompactionSummaryPlaceholder({
    conversationInput,
    compactionSummaryInput: args.compactionSummaryInput,
  });
  if (
    args.preCompactionInput.length > 0 &&
    areEquivalentValues(
      replayConversationInput.slice(0, args.preCompactionInput.length),
      args.preCompactionInput,
    )
  ) {
    replayConversationInput = replayConversationInput.slice(
      args.preCompactionInput.length,
    );
  }
  return {
...
function findReplayMatch<TApi extends Api>(args: {
  ...
  const compactionSummaryInput = serializeMessagesToResponsesInput(args.model, [
    args.compactionSummaryMessage,
  ]);
  const preCompactionVariants = [
    ...createReplayVariants({
      model: args.model,
      entries: args.preCompactionEntries,
    }),
    createReplayMessageSet(args.model, []),
  ];
  const postCompactionVariants = createReplayVariants({
    model: args.model,
    entries: args.postCompactionEntries,
  });
  ...
  for (const preCompactionKept of preCompactionVariants) {
    for (const postCompactionTail of postCompactionVariants) {
      ...
      const prefixMatches = areEquivalentValues(
        args.payloadInput.slice(0, expectedBeforeTrailing.length),
        expectedBeforeTrailing,
      );
      const trailingMatches = areEquivalentValues(
        args.payloadInput.slice(tailEndIndex),
        args.freshPreamble.trailingInput,
      );
        ...
        const actualPostCompactionTail = cloneResponsesInputSlice(
          args.payloadInput.slice(
            args.freshPreamble.leadingInput.length +
              ...
              preCompactionKept.input.length,
            tailEndIndex,
          ),
        );
...
function buildNativeReplaySegmentsInternal<TApi extends Api>(args: {
  ...
  const contextPostCompactionTailMessages = collectReplayMessages(
  ...
  if (!replayMatch) {
    const compactionSummaryInput = serializeMessagesToResponsesInput(
      args.model,
      [compactionSummaryMessage],
    );
    const lenientReplay = buildLenientNativeReplayPayload({
      ...
      compactedWindow,
      compactionSummaryInput,
      preCompactionInput: serializeMessagesToResponsesInput(
        args.model,
        collectReplayMessages(preCompactionEntries),
      ),
    });
    if (lenientReplay) {
      return {
        ...
        segments: {
          ...
          originalPiReplayInput:
            cloneResponsesInputSlice(args.payload.input) ?? [],
