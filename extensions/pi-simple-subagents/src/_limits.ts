...
export function parseSubagentLimits(value: unknown): SubagentLimitsConfig {
	...
	const maxDepth = boundedInteger(value.maxDepth, "maxDepth", 1, HARD_MAX_DELEGATION_DEPTH);
	...
	if (value.persistChildSessions !== undefined && typeof value.persistChildSessions !== "boolean") {
...
export function loadSubagentLimits(filePath: string): SubagentLimitsConfig {
	...
	try {
		...
		throw new Error(`Failed to read subagent limits at ${filePath}: ${error instanceof Error ? error.message : error}`);
	}

	try {
		return parseSubagentLimits(JSON.parse(content));
	} catch (error) {
		if (error instanceof SyntaxError) throw new Error(`Invalid JSON in subagent limits at ${filePath}: ${error.message}`);
		throw error;
	}
}
