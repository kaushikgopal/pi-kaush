...
class JsonlTranscriptArtifact implements TranscriptArtifact {
	...
	append(stream: "stdout" | "stderr", data: string): boolean {
		...
		return this.stream.write(`${JSON.stringify({ record: "stream", at: Date.now(), stream, data })}\n`);
	}

	resumeWhenWritable(callback: () => void): void {
		if (this.streamError || this.stream.destroyed || !this.stream.writableNeedDrain) {
...
export function createTranscriptArtifact(agentDir: string, metadata: TranscriptMetadata): TranscriptArtifact {
	try {
		const rootKey = createHash("sha256").update(metadata.rootSessionId).digest("hex").slice(0, 16);
		...
		return new UnavailableTranscriptArtifact(error instanceof Error ? error.message : String(error));
	}
}
...
export function truncateUtf8Head(value: string, maxBytes: number): { value: string; truncated: boolean } {
...
export function truncateUtf8Tail(value: string, maxBytes: number): { value: string; truncated: boolean } {
	const bytes = Buffer.from(value, "utf8");
	if (bytes.length <= maxBytes) return { value, truncated: false };
	...
	for (let offset = start; offset <= Math.min(bytes.length, start + 3); offset++) {
		...
		if (decoded || offset === bytes.length) return { value: decoded, truncated: true };
	}
	return { value: "", truncated: true };
}

export function appendBoundedJsonValue<T>(values: T[], value: T, maxBytes: number): boolean {
	const valueBytes = Buffer.byteLength(JSON.stringify(value), "utf8");
	if (valueBytes > maxBytes) return true;

	values.push(value);
	let totalBytes = values.reduce((total, item) => total + Buffer.byteLength(JSON.stringify(item), "utf8"), 0);
	let truncated = false;
	while (values.length > 0 && totalBytes > maxBytes) {
		const removed = values.shift();
		if (removed !== undefined) totalBytes -= Buffer.byteLength(JSON.stringify(removed), "utf8");
		truncated = true;
	}
	return truncated;
}
 ...
 */
export function resolveSessionFilePath(agentDir: string, childCwd: string, sessionId: string): string | undefined {
	const sessionsRoot = path.join(agentDir, "sessions");
	const suffix = `_${sessionId}.jsonl`;
	const findInBucket = (bucketPath: string): string | undefined => {
		try {
			const match = fs.readdirSync(bucketPath).find((file) => file.endsWith(suffix));
	...
	try {
		const direct = findInBucket(path.join(sessionsRoot, encodeSessionBucket(fs.realpathSync(childCwd))));
