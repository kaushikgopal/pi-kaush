...
export function formatProfileEligibilityError(
	...
	const scope = scopeSize === undefined ? "" : ` (${scopeSize} models in scope)`;
	return [
...
export function formatProfileAttemptSummaries(
	...
	const parts = attempts.map((attempt) => {
		...
		const rawReason = attempt.errorMessage?.trim() || `exit code ${attempt.exitCode}`;
		const reason = rawReason.length > 200 ? `${rawReason.slice(0, 197)}...` : rawReason;
...
export function formatProfileCandidate(candidate: SubagentProfileCandidate): string {
