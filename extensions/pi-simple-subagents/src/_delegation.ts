...
export type ModelAliasThinkingLevel = "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max";
...
function parseModelSpec(modelSpec: string): { model: string; thinkingLevel?: ModelAliasThinkingLevel } {
...
function formatModelReference(model: AvailableModel, thinkingLevel?: ModelAliasThinkingLevel): string {
...
function versionDistance(a: number[] | undefined, b: number[] | undefined): number {
	if (!a || !b) return Number.POSITIVE_INFINITY;
	const length = Math.max(a.length, b.length);
	let distance = 0;
	for (let i = 0; i < length; i++) distance = distance * 1000 + Math.abs((a[i] ?? 0) - (b[i] ?? 0));
	return distance;
}
 ...
 */
function selectBestModel(query: string, models: ReadonlyArray<AvailableModel>): AvailableModel | undefined {
	const lowerQuery = query.toLowerCase();
	const exact = models.find(
		(model) => model.id.toLowerCase() === lowerQuery || model.name?.toLowerCase() === lowerQuery,
	);
	...
	if (normalizedQuery) {
		const normalizedExact = models.find((model) =>
			[model.id, model.name].some((name) => name && normalizeModelName(name) === normalizedQuery),
		);
	...
	const sameFamily = models
		...
		.filter(({ model }) => [model.id, model.name].some((name) => name && brandOf(name) === queryBrand));
	...
	ranked.sort(
		(a, b) => a.distance - b.distance || b.newest - a.newest || a.index - b.index,
	);
...
export function currentDelegationDepth(env: NodeJS.ProcessEnv = process.env): number {
...
export function hasDelegatedToolActivity(
	messages: readonly { role: string; content?: string | readonly { type: string }[] }[],
): boolean {
...
export function sessionIdFromJsonEvent(event: unknown): string | undefined {
	...
	return candidate.type === "session" && typeof candidate.id === "string" && candidate.id.length > 0
		? candidate.id
		: undefined;
}
