...
describe("subagent transcript artifacts", () => {
  test("writes complete private stdout and stderr JSONL records with an opaque filename", async () => {
    ...
    try {
      ...
      expect(records[2]).toMatchObject({
        record: "stream",
        stream: "stderr",
        data: "warning: full diagnostic\n",
      });
      expect(records[3]).toMatchObject({
        record: "stream",
        stream: "stdout",
        data: largeDiagnostic,
      });
  ...
  test("keeps recent JSON values within the configured byte cap", () => {
    ...
    expect(
      appendBoundedJsonValue(values, { id: 1, text: "a".repeat(20) }, 80),
    ).toBe(false);
    expect(
      appendBoundedJsonValue(values, { id: 2, text: "b".repeat(20) }, 80),
    ).toBe(false);
    expect(
      appendBoundedJsonValue(values, { id: 3, text: "c".repeat(20) }, 80),
    ).toBe(true);
    ...
    expect(
      appendBoundedJsonValue(values, { id: 4, text: "x".repeat(100) }, 80),
    ).toBe(true);
    expect(values).toEqual(beforeOversized);
  });

  test("truncates UTF-8 previews without splitting characters", () => {
    expect(truncateUtf8Head("a🙂b", 5)).toEqual({
      value: "a🙂",
      truncated: true,
    });
    expect(truncateUtf8Tail("a🙂b", 5)).toEqual({
      value: "🙂b",
      truncated: true,
    });
    expect(truncateUtf8Head("short", 20)).toEqual({
      value: "short",
      truncated: false,
    });
  });
