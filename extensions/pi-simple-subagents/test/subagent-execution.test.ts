...
class FakeScheduler {
	...
	fire(delayMs: number) {
		const match = Array.from(this.tasks.entries()).find(([, task]) => task.delayMs === delayMs);
...
describe("subagent execution watchdog", () => {
	test("resets only the inactivity deadline when output arrives", () => {
		...
		const watchdog = createSubagentExecutionWatchdog(limits, (reason) => reasons.push(reason), scheduler);
