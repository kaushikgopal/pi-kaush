...
const { registerSubagent } = await import("../src/subagent");
...
function registerTestTool(): { tool: any; tools: any[]; handlers: Record<string, () => Promise<void>> } {
	let tool: any;
	const tools: any[] = [];
...
describe("subagent profile tool surface", () => {
	test("publishes profiles in every mode and renders the selected profile in brackets", () => {
		...
		const expectedProfiles = ["quick", "coder", "default", "thinker", "deep-thinker"];
		expect(tool.parameters.properties.profile.enum).toEqual(expectedProfiles);
		expect(tool.parameters.properties.tasks.items.properties.profile.enum).toEqual(expectedProfiles);
		expect(tool.parameters.properties.chain.items.properties.profile.enum).toEqual(expectedProfiles);
	...
	test("registers yield only inside delegated child sessions", () => {
		...
		try {
			...
			expect(registerTestTool().tools.map((tool) => tool.name)).toEqual(["subagent"]);

			process.env.PI_SUBAGENT_DEPTH = "1";
			expect(registerTestTool().tools.map((tool) => tool.name)).toEqual(["yield", "subagent"]);
		} finally {
	...
	test("hides the profile badge when an explicit model overrides it", () => {
		...
		const rendered = tool.renderCall(
			{ agent: "bee", task: "Do the work", profile: "deep-thinker", model: "provider/model" },
			theme,
			{ cwd: "/tmp", state: {} },
		) as { text: string };
	...
	test("rejects shared context outside parallel mode", async () => {
		...
		expect(result.content[0].text).toContain("context is supported only with parallel tasks[]");
	});
	...
	test("treats empty shared context as omitted in single mode", async () => {
		...
		const result = await tool.execute(
			...
			undefined,
			{
				...
				scopedModels: [],
				modelRegistry: { getAvailable: () => [] },
				sessionManager: { getSessionId: () => "root-session", getEntries: () => [] },
			},
		);
	...
	test("blocks execution beyond the hard delegation depth", async () => {
		...
		try {
			...
			expect(result.details.concurrency).toMatchObject({ active: 0, limit: 5, available: 5 });
		} finally {
			if (previousDepth === undefined) delete process.env.PI_SUBAGENT_DEPTH;
			else process.env.PI_SUBAGENT_DEPTH = previousDepth;
		}
