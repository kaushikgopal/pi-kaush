...
} from "../src/_delegation";
import {
	formatAgentDisplayName,
	formatProfileDisplayName,
	resolveAgentDisplayName,
} from "../src/_display";
import type { SubagentLimitsConfig } from "../src/_limits";
...
describe("subagent model selection", () => {
	test("prefers an invocation model over the agent default", () => {
		expect(resolveRequestedModel("provider/task-model:high", "provider/default-model")).toBe(
			"provider/task-model:high",
		);
		expect(resolveRequestedModel("  ", "provider/default-model")).toBe("provider/default-model");
	...
	test("resolves a bare model id or display name to the first available match", () => {
		...
		expect(resolveModelReference("gpt-5.6-luna", available)).toBe("provider-a/gpt-5.6-luna");
		expect(resolveModelReference("GPT-5.6 Luna", available)).toBe("provider-a/gpt-5.6-luna");
	});

	test("preserves qualified references and thinking levels", () => {
		const available = [{ provider: "provider-a", id: "gpt-5.6-luna" }];
		expect(resolveModelReference("provider-a/gpt-5.6-luna", available)).toBe("provider-a/gpt-5.6-luna");
		expect(resolveModelReference("gpt-5.6-luna:high", available)).toBe("provider-a/gpt-5.6-luna:high");
		expect(resolveModelReference("provider-a/gpt-5.6-luna:high", available)).toBe(
			"provider-a/gpt-5.6-luna:high",
		);
	});

	test("delegation resolves Kimi K3 from the enabled model scope", () => {
		const available = [{ provider: "huggingface", id: "moonshotai/Kimi-K3", name: "Kimi K3" }];
		const scoped = [{ model: { provider: "fireworks-kimi", id: "kimi-k3-fast", name: "Kimi K3 Fast" } }];
		const resolveModel = createModelResolver(delegationModelCandidates(scoped, available));
	...
	test("falls back to the in-scope family member when the exact version is unavailable", () => {
		const available = [{ provider: "provider-b", id: "glm-5.6", name: "GLM 5.6" }];
		const scoped = [{ model: { provider: "provider-a", id: "glm-5.2-fast", name: "GLM 5.2 Fast" } }];
		const resolveModel = createModelResolver(delegationModelCandidates(scoped, available));
		expect(resolveRequestedModel("GLM 5.6", undefined, resolveModel)).toBe("provider-a/glm-5.2-fast");
	});

	test("does not cross model families when the requested version is absent", () => {
		const scoped = [{ model: { provider: "provider-a", id: "glm-5.2-fast", name: "GLM 5.2 Fast" } }];
		const resolveModel = createModelResolver(delegationModelCandidates(scoped, []));
		expect(resolveRequestedModel("GPT 5.6", undefined, resolveModel)).toBeUndefined();
	});
	...
	test("picks the nearest available version within a family", () => {
		...
		const resolveModel = createModelResolver(delegationModelCandidates(scoped, []));
		expect(resolveRequestedModel("glm 5.6", undefined, resolveModel)).toBe("zai/glm-5.8-fast");
		expect(resolveRequestedModel("glm 5.3", undefined, resolveModel)).toBe("zai/glm-5.2-fast");
	});
	...
	test("prefers an exact version over nearby versions in the same family", () => {
		...
		const resolveModel = createModelResolver(delegationModelCandidates(scoped, []));
		expect(resolveRequestedModel("glm 5.6", undefined, resolveModel)).toBe("zai/glm-5.6-fast");
	});
	...
	test("a brand-only query resolves to the newest same-family candidate", () => {
		...
		const resolveModel = createModelResolver(delegationModelCandidates(scoped, []));
		expect(resolveRequestedModel("glm", undefined, resolveModel)).toBe("zai/glm-5.8-fast");
	});

	test("a brandless version query does not guess across families", () => {
		const scoped = [{ model: { provider: "zai", id: "glm-5.2-fast", name: "GLM 5.2 Fast" } }];
		const resolveModel = createModelResolver(delegationModelCandidates(scoped, []));
		expect(resolveRequestedModel("5.6", undefined, resolveModel)).toBeUndefined();
	});
	...
	test("keeps registry order when multiple providers match", () => {
		...
		expect(resolveModelReference("gpt-5.6-luna", available)).toBe("provider-b/gpt-5.6-luna");
	});

	test("createModelResolver wires resolveRequestedModel", () => {
		const available = [{ provider: "provider-a", id: "gpt-5.6-luna" }];
		const resolveModel = createModelResolver(available);
		expect(resolveRequestedModel("gpt-5.6-luna", undefined, resolveModel)).toBe("provider-a/gpt-5.6-luna");
		expect(resolveRequestedModel(undefined, "gpt-5.6-luna", resolveModel)).toBe("provider-a/gpt-5.6-luna");
		expect(resolveRequestedModel("provider-a/gpt-5.6-luna", undefined, resolveModel)).toBe(
...
describe("project-agent approval frontmatter", () => {
	...
	test("defaults to confirmation when the preference is absent", () => {
		...
		expect(shouldConfirmProjectAgent({ ...agent, confirmProjectAgents: true })).toBe(true);
		expect(shouldConfirmProjectAgent({ ...agent, confirmProjectAgents: false })).toBe(false);
	});
});
...
describe("delegation lineage", () => {
	test("starts at depth one and preserves the inherited root", () => {
		...
		expect(createDelegationTrace("bee-session", "call-3", { PI_SUBAGENT_DEPTH: "2junk" }).depth).toBe(1);
	});
...
describe("delegated prompt boundary", () => {
	test("allows one bounded subdelegation level and prohibits herdr", () => {
		const trace = createDelegationTrace("main-session", "call-1", {});
		const prompt = buildDelegatedSystemPrompt("Agent-specific instructions.", trace, limits);
	...
	test("prohibits delegation at the hard maximum depth", () => {
		const trace = createDelegationTrace("bee-session", "call-2", { PI_SUBAGENT_DEPTH: "1" });
...
describe("parallel shared context", () => {
	test("prepends one immutable context block without changing the assigned task", () => {
		expect(buildSharedTaskPrompt("Inspect server changes", "  Preserve the public API.  ")).toBe(
			"Shared context:\nPreserve the public API.\n\nAssigned task:\nInspect server changes",
		);
		expect(buildSharedTaskPrompt("Inspect server changes")).toBe("Inspect server changes");
		expect(buildSharedTaskPrompt("Inspect server changes", "   ")).toBe("Inspect server changes");
	});
});
...
describe("profile fallback safety", () => {
	test("only treats actual delegated tool activity as a side-effect boundary", () => {
		...
		expect(hasDelegatedToolActivity([{ role: "toolResult", content: [] }])).toBe(true);
	});
});

describe("child session correlation", () => {
	test("extracts only JSON session header ids", () => {
		expect(sessionIdFromJsonEvent({ type: "session", id: "child-session" })).toBe("child-session");
		expect(sessionIdFromJsonEvent({ type: "message_end", id: "entry-id" })).toBeUndefined();
...
describe("child session naming", () => {
	test("builds a picker-friendly name from the agent and task preview", () => {
		expect(buildChildSessionName("bee", "fix the flaky auth test")).toBe("subagent(bee): fix the flaky auth test");
	});

	test("collapses whitespace and clips long tasks", () => {
		const longTask = `${"write more tests ".repeat(10)}done`;
		const name = buildChildSessionName("bee", `  ${longTask}\nwith\tmessy  whitespace `);
