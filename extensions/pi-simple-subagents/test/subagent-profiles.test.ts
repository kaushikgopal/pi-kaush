...
describe("subagent profiles", () => {
  test("loads the configured profile ladder in stable order", () => {
    ...
    try {
      ...
      expect(
        formatProfileCandidate(config.profiles["quick"]!.candidates[0]!),
      ).toBe("openai-codex/gpt-5.6-luna:minimal");
      expect(
        formatProfileCandidate(config.profiles["coder"]!.candidates[0]!),
      ).toBe("openrouter/moonshotai/kimi-k2.7-code");
      expect(
        formatProfileCandidate(config.profiles["default"]!.candidates[0]!),
      ).toBe("openrouter/z-ai/glm-5.3-flash");
      expect(
        formatProfileCandidate(config.profiles["thinker"]!.candidates[0]!),
      ).toBe("openrouter/anthropic/claude-opus-4.8:xhigh");
      expect(
        formatProfileCandidate(config.profiles["deep-thinker"]!.candidates[0]!),
      ).toBe("openrouter/anthropic/claude-fable-5:xhigh");
    } finally {
...
describe("profiles reload", () => {
  ...
  test("reloads profiles when the source file changes mid-session", () => {
    ...
    try {
      ...
      expect(second.config.profiles.coder!.candidates[0]?.model).toBe(
        "test-provider/model-1",
      );
  ...
  test("propagates parse errors instead of serving a stale snapshot", () => {
    ...
    try {
      ...
      expect(() => loadSubagentProfilesCurrent(file, loaded)).toThrow(
        /model profiles/i,
      );
    } finally {
      rmSync(dir, { recursive: true, force: true });
...
describe("profile diagnostics", () => {
  ...
  test("attempt summaries report each candidate and bounded reason", () => {
    const message = formatProfileAttemptSummaries("coder", [
      {
        requestedModel: "open-weights/glm-flash-latest",
        exitCode: 1,
        errorMessage: "  provider auth missing\nfor key  ",
      },
      {
        model: "open-weights/kimi-code-latest",
        exitCode: 0,
    ...
    expect(message).toContain(
      "open-weights/glm-flash-latest — provider auth missing for key",
    );
