...
import { SessionConcurrencyGate } from "../src/_concurrency";
...
describe("SessionConcurrencyGate", () => {
	test("queues above the active limit and releases slots on completion", async () => {
		...
		expect(gate.status).toEqual({ active: 2, queued: 1, limit: 2, available: 0 });
		...
		expect(gate.status).toEqual({ active: 2, queued: 0, limit: 2, available: 0 });

		releaseSecond();
		releaseThird();
		expect(gate.status).toEqual({ active: 0, queued: 0, limit: 2, available: 2 });
	});
