...
function firstJsonLine(proc: ChildProcess): Promise<{ leafPid: number }> {
  return new Promise((resolve, reject) => {
    ...
    proc.once("close", (code) =>
      reject(new Error(`Process closed before announcing its child (${code})`)),
    );
  });
}
...
describe("subagent process trees", () => {
  ...
  if (process.platform !== "win32") {
    ...
    test("forces a resistant nested descendant without killing its sibling", async () => {
      ...
      if (nested.pid === undefined || sibling.pid === undefined)
      ...
      try {
        ...
        await handle.done;
        await waitFor(
          () => !processExists(nestedPid) && !processExists(leafPid),
        );
    ...
    test("kills one forced subtree without affecting its sibling, then cleans up on shutdown", async () => {
      ...
      try {
        ...
        const [firstLeafPid, siblingLeafPid] = await Promise.all([
          first.leafPid,
          sibling.leafPid,
        ]);
        ...
        await waitFor(
          () =>
            !processExists(first!.parentPid) && !processExists(firstLeafPid),
        );
        ...
        await waitFor(
          () =>
            !processExists(sibling!.parentPid) &&
            !processExists(siblingLeafPid),
        );
        expect(registry.size).toBe(0);
