...
describe("structured subagent yield", () => {
  ...
  test("applies completed, blocked, and failed outcomes to the parent result", () => {
    ...
    expect(
      applySubagentYield(
        completed,
        yieldMessage("completed", "done", ["result.txt"]),
      ),
    ).toBe(true);
    ...
    for (const status of ["blocked", "failed"] as const) {
      const target = { output: "" };
      expect(
        applySubagentYield(target, yieldMessage(status, `${status} result`)),
      ).toBe(true);
  ...
  test("ignores malformed or unrelated tool results", () => {
    ...
    expect(
      subagentYieldFromMessage(
        yieldMessage("completed", "ok", new Array(21).fill("file")),
      ),
    ).toBeUndefined();
  });

  test("adds yield to explicit child tool allowlists without duplicates", () => {
    expect(includeSubagentYieldTool(["read", "edit"])).toEqual([
      "read",
      "edit",
      "yield",
    ]);
    expect(includeSubagentYieldTool(["read", "yield"])).toEqual([
      "read",
      "yield",
    ]);
  });
