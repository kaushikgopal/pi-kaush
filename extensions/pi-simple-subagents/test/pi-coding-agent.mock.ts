...
export function createPiCodingAgentMock() {
	return {
		...
		truncateHead(text: string, options: { maxBytes: number; maxLines: number }) {
		...
		createReadTool() {
			return {
				async execute(_id: string, params: { path: string }) {
					...
					return { content: [{ type: "text", text: `built-in:${params.path}` }], details: {} };
				},
			};
		},
		...
		withFileMutationQueue: async <T>(_path: string, operation: () => Promise<T>) => operation(),
		parseFrontmatter<T extends Record<string, unknown>>(content: string) {
			...
			for (const line of match[1].split("\n")) {
				...
				frontmatter[key] =
					raw === "true" ? true : raw === "false" ? false : raw.replace(/^["']|["']$/g, "");
			}
			return { frontmatter: frontmatter as T, body: normalized.slice(match[0].length) };
		},
	};
}
