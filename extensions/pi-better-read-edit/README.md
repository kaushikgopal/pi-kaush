...
- Unsure about line numbers? Add `oldText` — the exact current lines, copied from the read output and appearing exactly once in the file — with `newText` as the replacement (omit `newText` to delete).
- A splice with both `startLine` and `oldText` tries the coordinates first and falls back to the unique text match when they fail; the fallback needs `deleteCount` of at least 1 and is reported as a warning.
  Repeat file entries to edit multiple files, using each file's own tag. Every splice uses original-snapshot coordinates, so earlier entries never shift later ones. A text-matched splice locates its lines by exact content in the live file, so a wrong line count cannot corrupt an unintended region. Overlapping ranges, duplicate insertion boundaries, inserts beside changed spans, no-ops, unseen boundaries or lines, ambiguous or missing text anchors, coordinate splices whose target content appears more than once in the file (use a wider unique span or an oldText anchor), and unrecoverable stale files are rejected before writing. Bounded JSON-string encodings of `files` or `edits` are normalized before the same strict schema validation for provider compatibility. Stored calls using the original `script` format are also converted before validation, but new calls see only the structured schema.
...
- **No fuzzy modes:** exact-text splices match a unique literal line sequence or fail; stale recovery maps requested operations through unique unchanged neighboring context. There is no apply-patch fallback, fuzzy or best-effort matching, syntax-block selector, register, move operation, or heuristic tag repair.
